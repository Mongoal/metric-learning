...
def show_spect_and_enengy():
    path = r'E:\IS95_data\ha_wch_is95_1'
    os.listdir(path)
    files = os.listdir(path)
    paths = [os.path.join(path, file) if file.endswith('iq') else None for file in files]
    ori_sig = read_dat(paths[2])
    origin_signal = ori_sig
    analyze = origin_signal[:, 0] + np.asarray(1j, np.complex64) * origin_signal[:, 1]
    ...
    for i in range(energy_len):
        ...
        # 能量块的能量定义为， 能量块内信号幅度平方和的开方根，若gate 为1000 ，则对应平均幅度为sqrt(gate**2 /100) = gate /10 = 100
        energy[i] = np.linalg.norm(analyze[head:tail])/np.sqrt(window_size)
    f, t, sxx = Sig.spectrogram(analyze,nperseg=256,noverlap=-window_size,nfft=256)
    plt.figure(figsize=(50,5))
    plt.subplot(211)
    plt.pcolormesh(np.log10(np.fft.fftshift(sxx, axes=0)), shading='gouraud')
...
def energy_detect_N_cut_origin(origin_signal, window_size=100, gate=1e2, num_win_per_sample=30,
                               drop_abnormal_mean=False,save_enengy_fig=None):
    '''
    能量检测，时域切割
    :param origin_signal: numpy ndarray, shape = [Num, 2]，原始时域信号, I、Q两路
    :param window_size:
    ...
    if save_enengy_fig:
        ...
        plt.ylabel('Energy')
        plt.xlim([0, len(energy)])
        plt.savefig(f'enengy_fig/{save_enengy_fig}.png')
...
def make_dataset_cut_origin_signal(directory, pattern='**/*.dat', outpath='LTE_origin_3240_dataset_5c_10s_1202.h5'):
    ...
    dset = h5f.create_dataset('dirs', (len(docs),), dtype=dt)
    for label, doc in enumerate(docs):
        dset[label] = doc
        
        save_dir = f'enengy_fig/{doc}/'
        ...
        for file in files:
            ...
            samples = energy_detect_N_cut_origin(sig, window_size=500,
                                                 gate=15,
                                                 num_win_per_sample=10,
                                                 save_enengy_fig=None)
                                                 # save_enengy_fig=f'{doc}/{os.path.basename(file)}')
...
def exstft(frame, nfft=8192, nstart=1184, nend=1291):

    f,t,zxx = Sig.stft(frame,nperseg=128,noverlap=64,nfft=nfft)
    zxx = zxx[nstart:nend,:]
    z = zxx / np.max(abs(zxx))
...
def myfft2(signal, nperseg=64, nfft=128, noverlap=44, _resize=True):
    _, _, z = Sig.stft(signal, nperseg=nperseg, nfft=nfft, noverlap=noverlap)
    z = np.fft.fftshift(z, 0)[:, :len(z)]
    z = z / np.max(abs(z))
    feature = np.zeros((z.shape[0], z.shape[1], 4), np.float32)
    feature[:, :, 0] = z.real / 2 + 0.5
    feature[:, :, 1] = z.imag / 2 + 0.5
...
def myfft1(signal, nperseg=64, nfft=256, noverlap=52,ignoreme=None):
    signal = signal / np.linalg.norm(signal) * len(signal)
    _, _, z = Sig.stft(signal, nperseg=nperseg, nfft=nfft, noverlap=noverlap, return_onesided=True)
    z = np.fft.fftshift(z, 0)[:, z.shape[0]]
    z = z / np.max(abs(z))
    feature = np.zeros((z.shape[0], z.shape[1], 4), np.float32)
    feature[:, :, 0] = z.real
    feature[:, :, 1] = z.imag
...
#     make_dataset_signal_fc(DATA9_ROOT, drop_abnormal_mean=True, outpath="dataset_signal_5000_new.h5")
    ...
    make_dataset_cut_origin_signal(directory=r'E:\IS95_data\lca_1',
                                  pattern='**/*.iq',
                                  outpath='../data/is95_cdma_8c_5000_20201118.h5')