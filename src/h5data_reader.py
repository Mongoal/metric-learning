...
class H5DataReader(object):
    ...
    def __init__(self, file, mode='r', data_key='features', label_key='labels', seed=0, train_set_ratio=0.7,
        ...
        # 如果文件小于1个G则全部读入内存
        if os.path.getsize(file) < 20 * 1024 * 1024 * 1024:
        ...
        self.set_idx(train_set_ratio=train_set_ratio, seg_set_method=seg_set_method, value=txt_path)

    def __str__(self):
        return str(self._file)

    def set_idx(self, train_set_ratio=0.7, seg_set_method='shuffle', value=None):
        self.shuffle_indices = np.random.permutation(len(self._labels))
        if seg_set_method == 'continuous':
            self.train_indices = np.random.permutation(int(len(self.shuffle_indices) * train_set_ratio))
            self.test_indices = np.arange(int(len(self.shuffle_indices) * train_set_ratio), len(self._labels))
        elif seg_set_method == 'shuffle':
            if (value is not None and os.path.isfile(value)):
                self.shuffle_indices = np.loadtxt(value, dtype=np.int32, delimiter='\n')
            ...
            self.shuffle_indices = np.random.permutation(np.loadtxt(value, dtype=np.int32, delimiter='\n'))
            self.train_indices = self.shuffle_indices[:int(len(self.shuffle_indices) * train_set_ratio)]
            self.test_indices = self.shuffle_indices[int(len(self.shuffle_indices) * train_set_ratio):]
        elif seg_set_method == 'array':
            self.shuffle_indices = np.random.permutation(value)
    ...
    def set_condition_idx(self, condition_keys: list = ['labels', 'fc'],
        ...
        self.set_idx(seg_ratio, seg_set_method='array', value=idx)
    ...
    def get_condition_idx(self, condition_keys: list = ['labels', 'fc'],
        ...
        included = np.ones(len(self._labels), dtype=np.bool)
        # 如果没有指定include条件（白名单），默认整个数据集
        if include_conditions:
            # 如果有include条件，只留下白名单的编号
            if len(condition_keys) > 1 and type(condition_keys) != str:
                combines = zip(*[self._file[key][:] for key in condition_keys])
                included = np.asarray([list(e) in include_conditions for e in combines])
            else:
                key = condition_keys[0]
                included = np.asarray([e in include_conditions for e in self._file[key][:]])

        # 在白名单里除去黑名单
        if exclude_conditions:
            if len(condition_keys) > 1 and type(condition_keys) != str:
                combines = zip(*[self._file[key][:] for key in condition_keys])
                not_excluded = np.asarray([list(e) not in exclude_conditions for e in combines])
            else:
                key = condition_keys[0]
                not_excluded = np.asarray([e not in exclude_conditions for e in self._file[key][:]])
        ...
        if seg_ratio is not None:
            idx = np.random.permutation(idx)
            num = int(seg_ratio * len(self._labels))
            idx = (idx[:num], idx[num:])
        ...
        print("# condition key: %s, include: %s, exclude: %s" % (
            condition_keys, include_conditions, exclude_conditions))
        return idx

    def set_limit_per_class(self, limit):
        if self.shuffle_indices is None:
            self.shuffle_indices = np.random.permutation(len(self._labels))
        labels = self._labels[:]
        count = np.zeros(labels.max() + 1)
        idxes = []
        for idx in self.shuffle_indices:
            if count[labels[idx]] < limit:
                count[labels[idx]] += 1
                idxes.append(idx)
        idxes = np.asarray(idxes)
        print("limit %d samples per class " % limit)
        self.set_idx(seg_set_method='array', value=idxes)
    ...
    def get(self,indexes):
        if type(indexes) == list:
            return [self._data[idx] for idx in indexes]
